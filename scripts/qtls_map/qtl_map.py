...
def main():
    ...
    if config_class._xqtlsMap == 1:
        ...
        print("\t".join(["uniqID", "db", "tissue", "protein", "testedAllele", "beta", "P", "type", "qtl_type", "genomicriskloci", "originalPhenotype"]), file=tmp_out)
        ...
        try:
            xqtls = xqtls[["uniqID", "db", "tissue", "protein", "type", "qtl_type", "genomicriskloci", "ensemble_id", "originalPhenotype"]]
            xqtls.to_csv(out_fp, sep='\t', encoding='utf-8', index=False, header=True)
        except:
            with open(out_fp, 'w') as f:
                f.write("uniqID\tdb\ttissue\tprotein\ttype\tqtl_type\tgenomicriskloci\tensemble_id\toriginalPhenotype\n")
