...
class Snp2geneProcess implements ShouldQueue
{
    ...
    private function coloc()
    {
        $jobID = $this->jobID;
        Storage::append($this->logfile, "----- coloc.py -----\n");
        Storage::append($this->errorfile, "----- coloc.py -----\n");

        $container_name = DockerNamesBuilder::containerName($jobID);
        $image_name = DockerNamesBuilder::imageName('laradock-fuma', 'qtl_map');
        $job_location = DockerNamesBuilder::jobLocation($jobID, 'snp2gene');

        $cmd = "docker run --rm --net=none --name " . $container_name . " -v $this->ref_data_path_on_host:/data -v " . config('app.abs_path_to_jobs_dir_on_host') . ":" . config('app.abs_path_to_jobs_dir_on_host') . " " . $image_name . " /bin/sh -c 'python coloc.py $job_location >>$job_location/job.log 2>>$job_location/error.log'";
        Storage::append($this->logfile, "Command to be executed:");
        Storage::append($this->logfile, $cmd . "\n");
